...
def main():
	...
	print(os.listdir('/mnt/finkbeinernas/robodata/Sina/data_sample/'),'\n')

	# Loop through subfolders in the dataset folder
	for folder in os.listdir('/mnt/finkbeinernas/robodata/Sina/data_sample/'):

		print(folder)
		# use re.match
		if 'condition_e_sample_B' in folder:
			#Running Bazel for prediction. Note txt log files are also being created incase troubleshooting is needed.
			date_time = datetime.now().strftime("%m-%d-%Y_%H:%M")
			dataset_eval_path = str(os.path.join('/mnt/finkbeinernas/robodata/Sina/data_sample/', folder))
